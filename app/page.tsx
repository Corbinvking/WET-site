...
function HeroStory({ story }: { story: typeof stories[0] }) {
  ...
  return (
    <article className="relative">
      {/* Large Hero Image */}
      <div className="relative aspect-[16/9] bg-gradient-to-br from-slate-100 to-slate-200 rounded-lg overflow-hidden mb-3">
      ...
      <Link href={`/story/${story.slug}`} className="group">
        <h2 className="headline-hero mb-2 group-hover:text-brand-primary transition-colors">
          {story.title}
        </h2>
      </Link>
      ...
      {/* Market Context - with green accent for positive */}
      {market && (
        <div className="flex items-center gap-3 text-xs border-t border-border pt-3">
          <span className="text-text-muted font-medium">{market.title}</span>
          <div className="flex items-center gap-2">
            <span className="px-2 py-1 bg-blue-50 text-brand-primary rounded font-semibold">
...
function LargeStoryCard({ story }: { story: typeof stories[0] }) {
  return (
    <article className="group">
      <div className="relative aspect-[4/3] bg-gradient-to-br from-slate-100 to-slate-200 rounded-lg overflow-hidden mb-2">
        <div className="absolute inset-0 flex items-center justify-center">
      ...
      <Link href={`/story/${story.slug}`}>
        <h3 className="headline-large group-hover:text-brand-primary transition-colors">
          {story.title}
        </h3>
      </Link>
    </article>
...
function StoryWithThumb({ story }: { story: typeof stories[0] }) {
  return (
    <article className="group flex gap-3 mb-4 last:mb-0">
      {/* Thumbnail */}
      <div className="flex-shrink-0 w-24 h-16 bg-gradient-to-br from-slate-100 to-slate-200 rounded overflow-hidden">
      ...
      <div className="flex-1">
        <Link href={`/story/${story.slug}`}>
          <h4 className="headline-compact group-hover:text-brand-primary transition-colors line-clamp-3">
            {story.title}
          </h4>
        </Link>
      </div>
...
function VideoPlaceholder() {
  return (
    <div className="relative aspect-video bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg overflow-hidden mb-3">
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-14 h-14 bg-brand-primary/90 rounded-full flex items-center justify-center hover:bg-brand-primary transition-colors cursor-pointer shadow-lg">
            ...
            <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-border">
              {subHeroStories.map((story) => (
                <article key={story.slug} className="group">
                  <div className="relative aspect-[4/3] bg-gradient-to-br from-slate-100 to-slate-200 rounded overflow-hidden mb-2">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <svg className="w-6 h-6 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                    ...
                    {story.labels.includes('Live Updates') && (
                      <div className="absolute top-2 left-2 flex items-center gap-1 px-1.5 py-0.5 bg-brand-red text-white text-[9px] font-bold uppercase rounded-sm">
